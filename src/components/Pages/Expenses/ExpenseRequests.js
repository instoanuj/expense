...
export async function getExpenses(dispatch) {
  const res = await axios.get(
    "https://trackexpense-cdef8-default-rtdb.firebaseio.com/expenses.json"
  );
  const data = [];
  let sum = 0;
...
export async function deleteExpense(id, dispatch) {
  await axios.delete(
    `https://trackexpense-cdef8-default-rtdb.firebaseio.com/expenses/${id}.json`
  );
...
export async function editExpense(id, item, dispatch) {
  const res = await axios.put(
    `https://trackexpense-cdef8-default-rtdb.firebaseio.com/${id}.json`,
    item         
  );
